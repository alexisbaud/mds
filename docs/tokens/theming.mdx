...
# Guide de Thématisation

MDS utilise des **variables CSS** pour faciliter la création de thèmes personnalisés. Vous pouvez override les tokens pour créer des thèmes dark, high contrast, ou entièrement personnalisés.

## Principe

Les tokens sont exposés comme variables CSS (`--token-name`). Pour créer un thème, il suffit de redéfinir ces variables dans un contexte spécifique.

## Thème global

### Override au niveau `:root`

Redéfinissez les variables CSS directement dans `:root` pour un changement global :
...
import './custom-theme.css';  // Après tokens.css
...
Utilisez des attributs comme `[data-theme]` pour des thèmes switchables :
...
## Thèmes partiels (scoped)

Vous pouvez créer des thèmes pour des sections spécifiques :

```css
/* Sidebar avec thème indépendant */
.sidebar {
...
```css
/* Auto dark mode basé sur préférences OS */
...
```css
[data-theme="dark"] {
  /* Redéfinir TOUS les tokens */
  --spacing-inset-sm: 12px;  /* Inutile si inchangé */
  --radius-md: 8px;          /* Inutile si inchangé */
}
```
...
Vérifiez toujours que vos paires `surface/content` respectent les cibles APCA. Consultez **[Contrast Pairs](/docs/tokens/contrast-pairs)**.
...
Créez un helper pour générer des thèmes programmatiquement :
...
- ✅ Override simple via `:root` ou `[data-theme]`

**Phase future :**
- 🔜 Dark theme officiel dans `semantic.json`
- 🔜 High contrast theme
- 🔜 Générateur de thèmes automatique
- 🔜 Support de plusieurs thèmes en parallèle

## Ressources

- **[Semantic Tokens](/docs/tokens/semantic)** : tokens disponibles pour override
- **[Contrast Pairs](/docs/tokens/contrast-pairs)** : vérifier les contrastes
