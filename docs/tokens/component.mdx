...
Les **component tokens** sont des recettes spécifiques à chaque composant. Ils composent les tokens sémantiques pour créer des décisions de design atomiques par composant.

## Rôle

Les component tokens permettent de :
- **Centraliser** les décisions de design par composant
- **Faciliter** la cohérence entre variantes d'un même composant
- **Simplifier** les modifications de design (une seule source de vérité)
- **Découpler** les composants des tokens sémantiques (abstraction)

## Principe

**Les component tokens référencent UNIQUEMENT des tokens sémantiques.**

```
Primitives → Brand → Sémantiques → Component → UI
```

## Structure

Les component tokens sont organisés par composant, puis par propriété :
...
Les component tokens documentent explicitement les décisions de design par composant.

## Workflow de création

### 1. Créer le composant avec tokens sémantiques
...
Si plusieurs variantes utilisent les mêmes combinaisons, créer des component tokens.
...
Les component tokens sont créés **au fil de l'eau** lors du développement des composants.

## Quand créer des component tokens ?

✅ **Oui :**
- Le composant a plusieurs variantes avec patterns similaires
- Les mêmes combinaisons de tokens sémantiques reviennent souvent
- Le composant est réutilisé dans de nombreux endroits
- Les décisions de design doivent être centralisées

❌ **Non :**
- Le composant est très simple (un seul style)
- Les tokens sémantiques suffisent
- Le composant est unique et expérimental
